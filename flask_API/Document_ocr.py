...
class OCR():
    ...
    def img_process(self,file_save_path):

        self.crop_file_save_path = file_save_path + "crop_image/"
        self.result_file_save_path = file_save_path + "result_image/"

        sr = cv2.dnn_superres.DnnSuperResImpl_create()
        sr.readModel(file_save_path + 'model/ESPCN_x2.pb') 
        sr.setModel("espcn", 2)
        ...
        for k in range(self.detect_large_box_num):
            ...
            for m in range(len(small_rect_list4)):
                ...
                if W > (crop_max_width*3/100) and H >(crop_max_height*3/100) and W>H and H <(crop_max_height*60/100): # 예외처리
                    ...
                    cv2.imwrite(self.crop_file_save_path+"ocr_crop_image_{}_{}.png".format(k,image_num), ocr_crop_image)
    ...
    def ocr_process(self):
        ...
        for i in range(2):
            ...
            for k in box_num_list:
                path = self.crop_file_save_path + "ocr_crop_image_"+str(i)+"_"+str(k)+".png"
                ...
                for j in type2_text:
                    ...
                    if i == 0 and k == 20:      # masking을 위한 20번째 박스안에서 text가 있는 부분의 좌표 추출
                        ...
                        cv2.imwrite(self.result_file_save_path+"masking_img.jpg",self.masking_img)  # masking img 저장하기
        ...
        ocr_data_frame.to_csv(self.result_file_save_path+"result.csv")
