...
upload_image = cv2.imread("/home/matrix/Desktop/code/AI_POC/document/car_document_0621_1.jpg", cv2.IMREAD_GRAYSCALE)
...
for k in range(n):
    ...
    #     biggest = biggest_contour(small_cnts[i:])
        
    #     if len(biggest) > 0:
    #         cv2.drawContours(crop_image, [biggest], -1, (255,0,0),3 )
    ...
    #         small_rect_list.append(input_points)
    ...
    # elif k == 1:
    #     height_list = [0,max_height*0.05853, max_height*0.1146, max_height*0.1573, max_height*0.1988,max_height*0.239,max_height*0.283,max_height*0.3598,max_height ]


    # for j in range(len(height_list)):
    #     small_rect_list2 = [small_rect_list[i] for i in range(len(small_rect_list)) 
    #                         if ((small_rect_list[i][0][1]+small_rect_list[i][3][1])/2) >= height_list[j] and ((small_rect_list[i][0][1]+small_rect_list[i][3][1])/2) <= height_list[j+1] ]
    ...
    # for k in range(len(small_cnts)):
    #     input_points3 = small_rect_list3[k]

    #     (top_left, top_right, bottom_left, bottom_right) = input_points3
    #     bottom_width = np.sqrt( ((bottom_right[0] - bottom_left[0])**2) + ((bottom_right[1] - bottom_left[1])**2) )
    #     top_width = np.sqrt( ((top_right[0] - top_left[0])**2) + ((top_right[1] - top_left[1])**2) )
    #     right_height = np.sqrt( ((top_right[0] - bottom_right[0])**2) + ((top_right[1] - bottom_right[1])**2) )
    #     left_height = np.sqrt( ((top_left[0] - bottom_left[0])**2) + ((top_left[1] - bottom_left[1])**2) )
    ...
    #     matrix = cv2.getPerspectiveTransform(input_points3, converted_points)
    #     crop_image2 = cv2.warpPerspective( crop_image , matrix, (max_width,max_height))

    #     cv2.imshow("crop_image", crop_image)
    #     cv2.imshow("OCR_crop_image_{}".format(k),crop_image2 )
